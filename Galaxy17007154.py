...
class Galaxy:
    def __init__(self, h_a, h_b, h_c, n_stars, seed =1):
        ...
        for i in range(self.n_stars):
            ran.append(random.random())
        a_max =  np.amax(-(self.h_a) * np.log((1 - np.array(ran)**(1/2))))
        ran = []
        for i in range(self.n_stars):
            ran.append(random.random())
        b_max =  np.amax(-(self.h_b) * np.log((1 - np.array(ran)**(1/2))))
        ...
        #Randomly finding a radius to the edge of the ellipsoid in the x-y plane

        rho  = (1 / (np.sin(star_theta)/a_max)**2 + (np.cos(star_theta)/b_max)**2)**(1/2)


        #using the radius found to find a combination of h_a and h_b to find a h_r value that is proportionl to the radius length.
        #e.g - if radius was equal to a, then h_r = h_a
        #e.g- if radius was eqaul to b, then h_r = h_b
        self.h_r = ((h_a*rho*np.sin(star_theta)/a_max)**2 + (h_b*rho*np.cos(star_theta)/b_max)**2)**(1/2)

        #Using the h_r value to radomly distribute along an exponential dist. 
        ...
        for i in range(len(rho)):
            while ( (b_max*rho[i]*np.sin(star_theta[i]))**2 + (a_max*rho[i]*np.cos(star_theta[i]))**2  - (a_max*b_max )**2 ) >= 0  :
                rho[i] = ( -(    (self.h_r[i] ) * np.log((1 - np.array(ran)**(1/2)))))
            else:
                pass
    ...
    def effected_by_LGRB_step_1(self, Number_LGRBs_step_1, SFR_total, SFR_100_70, SFR_70_50, SFR_50_40, SFR_40_30, SFR_30_20, SFR_20_10, SFR_10_0, E, T):
        ...
        #Finding the number of LGRBs in each section of the galaxy based on the SFR of each section. 
        ...
        #Finding phi values for the LGRBs
        ...
        #Distributing the LGRBs homogenously through the region specifiec
        ran = []
        for i in range(NumberLGRBs_100_70):
            ran.append(random.random())
        ...
        #70-50# (repeating the process for lower boundaries)
...
#Finding the dimenstions of the galaxy
...
#low metallicty fraction for up to redshift = 4, using Grham and Schady's metallicity evolution relationship
...
#Finding the number of LGRBs for the next time step
